...
allTask[clave++] = new tarea.Tarea("Tarea2", "Desc2",2,2);
allTask[clave++] = new tarea.Tarea("Tarea1", "Desc1",1,4);
allTask[clave++] = new tarea.Tarea("Tarea3", "Desc3",1,2);
allTask[clave++] = new tarea.Tarea("Tarea4", "Desc4",1,1);
allTask[clave++] = new tarea.Tarea("Tarea5", "Desc4",3,1);
allTask[clave++] = new tarea.Tarea("Tarea6", "Desc4",4,1);
allTask[clave++] = new tarea.Tarea("Tarea7", "Desc4",3,1);
allTask[clave++] = new tarea.Tarea("Tarea8", "Desc4",4,1);
...
tareaForm.onsubmit = (e) =>{
    ...
    let claveAdd = clave++;
    let valorAdd = new tarea.Tarea(nom,des,est,0);
...
elementosLista.addEventListener("click", (e) => {
    ...
    if(e.target.className.includes("cambiarLista")){

        if(numEstadoCont===1){
            let elementoLista = e.target.parentElement.parentElement;
        ...
        // BORRAR TAREA
    }else if(e.target.className.includes("borrarTareaI")){
        let tareaSeleccionada = e.target.parentElement.parentElement;
        ...
        // CAMBIAR DE ESTADO
    }else if(e.target.className.includes("cambiarTodo")){
        let tareaSeleccionada =  e.target.parentElement.previousElementSibling;
        let desplegable = e.target.parentElement;
        let clave = tareaSeleccionada.getAttribute("name");
        desplegable.remove();
        tareaSeleccionada.remove();
        cambiarEstado(clave, 1);

    }else if(e.target.className.includes("cambiarDoing")){
        let tareaSeleccionada =  e.target.parentElement.previousElementSibling;
        let desplegable = e.target.parentElement;
        let clave = tareaSeleccionada.getAttribute("name");
        desplegable.remove();
    ...
    }else if(e.target.className.includes("cambiarDone")){
        let tareaSeleccionada =  e.target.parentElement.previousElementSibling;
        let desplegable = e.target.parentElement;
        let clave = tareaSeleccionada.getAttribute("name");
        desplegable.remove();
